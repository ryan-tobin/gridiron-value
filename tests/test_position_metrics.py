...
def base(**values):
    row = {"season": 2026, "week": 1, "game_id": "g1", "gsis_id": "p1", "position": "QB", "attempts": 10, "sacks_suffered": 2, "passing_epa": 5.0, "passing_yards": 200, "completions": 6, "passing_tds": 2, "passing_interceptions": 1, "carries": 2, "rushing_epa": 1.0, "rushing_yards": 10, "rushing_20": 1, "targets": 0, "receptions": 0, "receiving_epa": 0.0, "receiving_yards": 0, "receiving_yards_after_catch": 0, "receiving_20": 0}
    row.update(values)
...
def test_qb_rates_use_explicit_denominators():
    ...
    assert result["pass_epa_per_attempt"] == 0.5
...
def test_receiver_metrics_and_role():
    result = derive_metrics(base(position="WR", attempts=0, sacks_suffered=0, carries=0, targets=5, receptions=4, receiving_epa=2, receiving_yards=60, receiving_yards_after_catch=20, receiving_20=1)).row(0, named=True)
