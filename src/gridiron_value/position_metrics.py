...
DEPRECATED_METRICS = {
    "pass_epa_per_attempt": (
        "Always null. Source passing_epa and attempts use different play "
        "population. Use EPA per eligible dropback computed from play-by-play."
    )
}
...
def derive_metrics(frame):
    ...
    if not required.issubset(frame.columns):
        raise ValueError(
            f"Player-game table missing: {sorted(required - set(frame.columns))}"
        )
    if (
        frame.select("season", "week", "game_id", "gsis_id").unique().height
        != frame.height
    ):
    ...
    role = (
        pl.when(pl.col("position").is_in(["QB"]))
        .then(pl.lit("QB"))
        .when(pl.col("position").is_in(["RB", "FB"]))
        .then(pl.lit("RB"))
        .when(pl.col("position").is_in(["WR"]))
        .then(pl.lit("WR"))
        .when(pl.col("position").is_in(["TE"]))
        .then(pl.lit("TE"))
    ...
    out = frame.with_columns(
        ...
        pl.lit(None, dtype=pl.Float64).alias("pass_epa_per_attempt"),
        ...
        safe_ratio(col("receiving_yards_after_catch"), receptions).alias(
            "yac_per_reception"
        ),
...
def main():
    ...
    if participation["source_player_game_manifest"] != h.record(root, player_game_path):
        raise ValueError(
            "Player-game and participation runs use different player-game inputs."
        )
    ...
    report.write_text(
        ...
        + ", ".join(
            f"{role}={metrics.filter(pl.col('metric_role') == role).height:,}"
            for role in ("QB", "RB", "WR", "TE", "OTHER")
        )
        + "\n\n"
        "Rates are calculated from the available player-game statistics. "
        "`pass_epa_per_attempt` is deprecated and always null because source passing EPA and attempts use different play populations. "
        "Other rates are null when the numerator is missing or the denominator is missing or nonpositive. "
